...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id = games.opponent_id WHERE games.round = 'Eighth-Final' AND games.year = 2014 UNION SELECT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE games.round = 'Eighth-Final' AND games.year = 2014")"
